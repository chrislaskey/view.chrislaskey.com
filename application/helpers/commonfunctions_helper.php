...
if( !function_exists('create_message') ){
	...
	function create_message($type, $message){
		...
		$session = $CI->session->userdata('flash:new:message');
		$messages = ( $session != NULL ) ? $session : array();
		$new = array();
		
		$type = strtolower($type);
		$types = array('error', 'notice', 'message');
		if( !in_array($type, $types) ){ $type = 'notice'; }
		
		$new['type'] = $type;
		$new['message'] = $message;
		array_push($messages, $new);
...
if( !function_exists('show_message') ){
	...
	function show_message($echo = TRUE, $new = FALSE){
		$CI = &get_instance();
		$CI->load->library('session');
		$session = ($new === TRUE) ? $CI->session->userdata('flash:new:message') : $CI->session->flashdata('message');
		
		if( is_array($session) && count($session) > 0 ){
			foreach($session as $s){
				$messages[] = '<div class="message message_type_'.$s['type'].'"><h2>'.ucwords($s['type']).'!</h2>'.$s['message'].' <a class="close" href="#">(Close)</a></div>';
			}unset($s);
			
			if( $echo === TRUE ){ echo implode($messages); }
			else{ return implode($messages); }
		}else{ return FALSE;}
	}
	
}

//* Function that returns message count.
