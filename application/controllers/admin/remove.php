...
class Remove extends CI_Controller {
	...
	function index(){
		...
		if( $this->uri->segment(3) && $this->view_images_model->is_image($this->uri->segment(3)) ){
			...
			create_message('error', 'Cannot remove image, image does not exist.');
			header('location:/');
		}
	}
	...
	function remove_image(){
		...
		if( is_numeric($id) ){
			...
			if( !$image = $this->view_images_model->return_image($id) ){
				create_message('error', 'Error retreiving image data from database.');
				return FALSE;
			}
			
			//Remove Database References
			if( !$this->view_images_model->delete_image($id) ){
				create_message('error', 'Error deleting image from database.');
				return FALSE;
			}
			
			if( !$this->view_times_model->delete_lookup($id) ){
				create_message('error', 'Error unlinking image from time set.');
				return FALSE;
			}
			
			if( !$this->view_tags_model->delete_lookup($id) ){
				create_message('error', 'Error unlinking image from tag set.');
				return FALSE;
			}
