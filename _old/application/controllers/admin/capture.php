...
class Capture extends Controller {
	...
	function _save(){
		...
		if( $image_url == NULL ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Image URL required.')); return;
		}elseif( $image_website == NULL ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Image Website required.')); return;
		}
		...
		if( !in_array($image_extension, $allowed_extensions) ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Unsupported file type.')); return;
		}unset($pathinfo);
		...
		//Verify data exists
		if( $raw_data == NULL ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'No data found.')); return;
		}
		...
		if( !is_numeric($image_id) ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating database entry.')); return;
		}
		...
		/*
		//Create large thumbnail (height 400)
		$config['height'] = 400;
		$config['width'] = round( ($config['height'] * $image_original_width) / $image_original_height);
		$image_height = $config['height'];
		$image_width = $config['width'];
		$config['maintain_ratio'] = TRUE;
		$config['source_image'] = DOCUMENT_ROOT . $image_original;
		$config['new_image'] = DOCUMENT_ROOT . $image_directory . $image_id . '_height_400.' . $image_extension;
		$this->load->library('image_lib', $config);
		if( !$this->image_lib->resize() ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating 400px high thumbnail.')); return;
		}unset($config);
		$this->image_lib->clear();
		
		//Create small thumbnail (width 160)
		$config['width'] = 160;
		$config['height'] = round( ($config['width'] * $image_original_height) / $image_original_width);
		$config['maintain_ratio'] = TRUE;
		$config['source_image'] = DOCUMENT_ROOT . $image_original;
		$config['new_image'] = DOCUMENT_ROOT . $image_directory . $image_id . '_width_160.' . $image_extension;
		$this->image_lib->initialize($config);
		if( !$this->image_lib->resize() ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating 160px width thumbnail.')); return;
		}unset($config);
		$this->image_lib->clear();
		
		//Create small thumbnail (height 60)
		$config['height'] = 60;
		$config['width'] = round( ($config['height'] * $image_original_width) / $image_original_height);
		$config['maintain_ratio'] = TRUE;
		$config['source_image'] = DOCUMENT_ROOT . $image_original;
		$config['new_image'] = DOCUMENT_ROOT . $image_directory . $image_id . '_height_60.' . $image_extension;
		$this->image_lib->initialize($config);
		if( !$this->image_lib->resize() ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating 60px high thumbnail.')); return;
		}unset($config);
		$this->image_lib->clear();
		*/
		...
		if( !file_exists(DOCUMENT_ROOT.FILE_DIRECTORY.floor($image_id / 100).'/'.$image_id.'_height_400.jpg') ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating 400 height thumbnail.')); return;
		}
		if( !file_exists(DOCUMENT_ROOT.FILE_DIRECTORY.floor($image_id / 100).'/'.$image_id.'_height_60.jpg') ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating 60 height thumbnail.')); return;
		}
		if( !file_exists(DOCUMENT_ROOT.FILE_DIRECTORY.floor($image_id / 100).'/'.$image_id.'_width_160.jpg') ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating 160 width thumbnail.')); return;
		}
		...
		if( !$this->view_images_model->update_image($image_id, $data) ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error updating database.')); return;
		}unset($data);
		...
		if( !$this->view_times_model->is_time($time_slug) ){
			...
			if( !$this->view_times_model->add_time($time) ){
				$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error creating time.')); return;
			}unset($time);
		}
		if( !$this->view_times_model->add_image_to_time($image_id, $time_slug) ){
			$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error adding image to time.')); return;
		}
		...
		if( count($tags) > 0 ){
			...
			foreach($tags as $tag){
				if( !$this->view_tags_model->add_image_to_tag($image_id, $tag) ){
					$this->session->set_flashdata('message', array('type'=>'error', 'message'=>'Error adding image to tag.')); return;
				}
			}
